...
//Bruno's Change - Added function to send the injection
void generateInjection(int producer_task, int consumer_task, unsigned int targetPE, unsigned int requestingPE, int insert_pipe_flag)
	ServiceHeader *p = get_service_header_slot();
	p->header = targetPE;
	...
	switch (service) {
		...
		case REALTIME:
			...
			if (producer_PE == net_address){ //Local producer
				...
				generateInjection(producer_task, consumer_task, producer_PE, net_address, 0);
