 ...
 *
 * \brief local_scheduler is a kernel module in charge of implementing
...
void real_time_task(Scheduling * real_time_task, unsigned int period, int deadline, unsigned int execution_time){
	...
	real_time_task->utilization = ((execution_time*100) / period) + 10; //10 is the inherent task overhead

	cpu_utilization += real_time_task->utilization;


#if DEBUG
	putsv("Address: ", (unsigned int) real_time_task);
...
void inline dynamic_slice_time(Scheduling *scheduled, unsigned int time){
	...
	for(int i=0; i<MAX_LOCAL_TASKS; i++){
		...
		if (task->status == SLEEPING || task->status == WAITING){
		...
		} else if (!second_LST || second_LST > task->slack_time){
...
void inline update_real_time(unsigned int current_time){
	...
	for(int i=0; i<MAX_LOCAL_TASKS; i++){
		...
		if (current_time >= (task->ready_time + task->period) ){
			...
			//Set to generate a deadline miss if the remaining executiong time is above 25% of the execution time
			if (task->status != WAITING && task->remaining_exec_time > (task->execution_time/4) ){
#if DEBUG
				putsv("## Deadline miss to task ", (unsigned int) task);
#endif
...
//putsv("\nSC: ",current_time);
	...
	for(int i=0; i<MAX_LOCAL_TASKS; i++){
		...
		if (task->status == READY && task->deadline != NO_DEADLINE){
